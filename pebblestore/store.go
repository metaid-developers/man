...
// PinSort: 分页信息独立pebble实例
// BlocksDB: 区块交易独立pebble实例
// CountDB: 一些缓冲的统计数据
// MetaidInfoDB: 存储metaid相关信息，key是metaid，value是metaid信息json
// PathPinDB：按pin_path数据，key是 path_blockTime_chainName_height_pinId,value是空
// AddressDB: 按地址存储的PIN数据，key是address转换后的metaid_path_blockTime_chainName_height_pinId,value是空
// PinsMempoolDb：内存池数据，key是 pinid,value是空
type Database struct {
	...
	MetaidInfoDB  *pebble.DB // 用于存储metaid信息
	MetaDb        *pebble.DB // 用于存储meta数据
	TransferDb    *pebble.DB // 用于存储转移数据
}
...
func NewDataBase(basePath string, shardNum int) (*Database, error) {
	...
	os.MkdirAll(fmt.Sprintf("%s/address", basePath), 0755)
	...
	os.MkdirAll(fmt.Sprintf("%s/metaidinfo", basePath), 0755)
	metaIdInfoDb, err := pebble.Open(fmt.Sprintf("%s/metaidinfo/db", basePath), dbOptions)
	if err != nil {
		log.Println(err)
		return nil, err
	}
	os.MkdirAll(fmt.Sprintf("%s/transfer", basePath), 0755)
	transferDb, err := pebble.Open(fmt.Sprintf("%s/transfer/db", basePath), dbOptions)
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func (idx *Database) BatchInsertPathPins(data *[]string) error {
	batch := idx.PathPinDB.NewBatch()
	for _, v := range *data {
		batch.Set([]byte(v), nil, pebble.Sync)
	}
	if err := batch.Commit(nil); err != nil {
		batch.Close()
		return err
...
func (idx *Database) BatchInsertTransferPins(data *[]*pin.PinTransferHistory) error {
	batch := idx.TransferDb.NewBatch()
	for _, v := range *data {
		content, err := sonic.Marshal(v)
		if err != nil {
			return err
		}
		key := fmt.Sprintf("%s_%s_%d", v.PinId, v.TransferTx, v.TransferTime)
		batch.Set([]byte(key), content, nil)
	}
	if err := batch.Commit(nil); err != nil {
		batch.Close()
		return err
...
func (idx *Database) BatchSetAddressData(data *[]string) error {
...
func (idx *Database) BatchMergeAddressData(height int64, data map[string]string) error {
	batch := idx.AddressDB.NewBatch()
	for k, v := range data {
...
func (idx *Database) GetMetaidInfo(metaId string) (*pin.MetaIdInfo, error) {
	var metaIdInfo pin.MetaIdInfo
	data, closer, err := idx.MetaidInfoDB.Get([]byte(metaId))
	if err != nil {
		if err == pebble.ErrNotFound {
			return &metaIdInfo, nil
		}
		return nil, err
	}
	defer closer.Close()
	if err := sonic.Unmarshal(data, &metaIdInfo); err != nil {
		return nil, err
	}
	return &metaIdInfo, nil
}
func (idx *Database) BatchSetMetaidInfo(data *map[string]*pin.MetaIdInfo) error {
	batch := idx.MetaidInfoDB.NewBatch()
	for k, v := range *data {
		content, err := sonic.Marshal(v)
		if err != nil {
			return err
		}
		batch.Set([]byte(k), content, nil)
	}
	if err := batch.Commit(nil); err != nil {
		batch.Close()
		return err
...
func (idx *Database) MetaIdPageList(page, size int64) (list []pin.MetaIdInfo, err error) {
	it, _ := idx.MetaidInfoDB.NewIter(nil)
	defer it.Close()
	skip := page * size
	count := int64(0)
...
func (idx *Database) InsertPinSort(db *pebble.DB, sortLsit []string) error {
	...
	for _, key := range sortLsit {
		batch.Set([]byte(key), nil, pebble.Sync)
	}
