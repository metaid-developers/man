...
type PinInscription struct {
	...
	OriginalContentBody    []byte          `json:"originalContentBody"`
	OriginalContentSummary string          `json:"originalContentSummary"`
...
func GenPinSortKey(p *PinInscription, blockTime int64, chainName string, height int64) string {
	...
	return common.ConcatBytesOptimized([]string{publicKeyStr, "&", common.GetMetaIdByAddress(p.Path), "&", p.MetaId, "&", p.Id}, "")
}
func GenPathSortKey(p *PinInscription, blockTime int64, chainName string, height int64) string {
	publicKeyStr := GetPublicKeyStr(blockTime, chainName, height)
	return common.ConcatBytesOptimized([]string{common.GetMetaIdByAddress(p.Path), "&", publicKeyStr, "&", p.Id}, "")
}
func GenAddressSortKey(p *PinInscription, blockTime int64, chainName string, height int64) string {
	publicKeyStr := GetPublicKeyStr(blockTime, chainName, height)
	return common.ConcatBytesOptimized([]string{p.MetaId, "&", common.GetMetaIdByAddress(p.Path), "&", publicKeyStr, "&", p.Id}, "")
}
